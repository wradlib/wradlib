...
def create_raster_xarray(
    ...
    crs = pyproj.CRS.from_user_input(crs).to_wkt(version="WKT2_2019")
...
def add_raster_grid_mapping(raster):
    if isinstance(raster, xr.DataArray):
        raster.attrs["grid_mapping"] = "spatial_ref"

    if isinstance(raster, xr.Dataset):
        for var in raster.data_vars:
            raster[var].attrs["grid_mapping"] = "spatial_ref"
