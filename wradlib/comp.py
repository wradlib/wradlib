...
def transform_binned(sweep, raster):
    ...
    radar_crs = georef.get_radar_crs(sweep)
    sweep = xradar.georeference.get_x_y_z(sweep)
    coord_sweep = np.dstack((sweep.x.values, sweep.y.values))
    coord_sweep = georef.transform_coords(coord_sweep, radar_crs, crs)

    coord_raster = georef.get_raster_coordinates(raster)
    ...
    if crs.is_geographic:
        coord_raster2 = georef.transform_coords(coord_raster, crs, radar_crs)
        center = (0, 0)
    else:
        center = georef.transform_coords((lon, lat), georef.wgs(), crs)
...
def sweep_to_raster(sweep, raster, **kwargs):
    ...
    **kwargs : dict
    ...
    georef.add_raster_grid_mapping(out)
