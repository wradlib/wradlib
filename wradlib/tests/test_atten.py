...
def get_gateset():
    return np.arange(2*2*5).reshape(2,2,5) * 3

def get_gateset_result():
    return np.array([[[  0.00000000e+00,   4.00000000e-04,   1.04876587e-03,
                         2.10105093e-03,   3.80794694e-03],
                      [  0.00000000e+00,   4.48807382e-03,   1.17721446e-02,
                         2.35994018e-02,   4.28175682e-02]],

                     [[  0.00000000e+00,   5.03570165e-02,   1.32692110e-01,
                         2.68007888e-01,   4.92303379e-01],
                      [  0.00000000e+00,   5.65015018e-01,   1.56873147e+00,
                         3.48241974e+00,   7.70744561e+00]]])

def test_calc_attenuation_forward():
    """basic test for correct numbers"""
    gateset = get_gateset()
    a = 2e-4
    b = 0.7
    l = 1.
    result = atten.calc_attenuation_forward(gateset, a, b, l)
    assert np.allclose(result, get_gateset_result())


def test_sector_filter_1():
...
def test_correctAttenuationConstrained2():
##    gateset = get_gateset()
##    a_max = 1.67e-4
##    a_min = 2.33e-5
##    na = 10
