...
def ex_beamblock(rasterfile, **kwargs):
    ...
    corners = mpl.transforms.Bbox(np.array([[lonmin, latmin], [lonmax,latmax]]))
    ...
    flist = sorted(glob.glob(path_to_gtopo + '[e,w]*.hdr'))
    found = []
    for file in flist:
        #ds = wrl.io.open_raster(file)
        print(file)
        hdr = np.loadtxt(file, dtype='S20')
        size = (int(hdr[2][1]),int(hdr[3][1]))
        ulx = float(hdr[10][1])
        uly = float(hdr[11][1])
        scale = (float(hdr[12][1]), float(hdr[13][1]))
    ...
    path_vrt = '/work/radproc/data/radar/gtopo30/data/'
    command=["gdalbuildvrt",  '-overwrite', '-te', str(lonmin-scale[0]),  str(latmin-scale[1]),
             str(lonmax+scale[0]), str(latmax+scale[1]), path_to_gtopo+ "interim1.vrt"]
    #command=["gdalbuildvrt",  '-overwrite', path_vrt+ "interim.vrt"]

    for file in found:
        (fn, ext) = os.path.splitext(file)

        command.append(fn+'.dem')

    print(command)
    #subprocess.call(command)
    #rasterfile = path_to_gtopo+'interim1.vrt'
    #_check_file(rasterfile)
    ...
    # read raster data
    #rastercoords,rastervalues = wrl.io.read_raster_data(rasterfile, driver='vrt', **kwargs)
    ...
    # plot terrain
    #dem = ax1.pcolormesh(rastercoords[...,0], rastercoords[...,1], rastervalues/1000., cmap=mpl.cm.terrain, vmin=-0.3, vmax=0.8)
...
if __name__ == '__main__':
    ...
    filename = os.path.join(filepath, 'bonn_gtopo.tif')
    #filename='/work/radproc/data/radar/gtopo30/data/w020n90.dem'
    filename='/work/radproc/data/radar/gtopo30/data/interim1.dem'
